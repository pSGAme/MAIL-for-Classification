...
class MultiModalMailLearner(nn.Module):
    ...
    def forward(self):
        return self.ln_proj_layers_mlp, self.ln_proj_layers_att, self.att_proj_layers_att, self.mlp_proj_layers_mlp
...
class CustomCLIP(nn.Module):
    ...
    def forward(self, image, tokenized_prompts, label=None):
        ...
        text_features = self.text_encoder(embedding, tokenized_prompts, ln_proj_layers_mlp,
...
class MAILSRC_Trainer(TrainerX):
    def __init__(self, cfg):
        ...
        self.prompts = [prompt_prefix + name + prompt_postfix for name in classnames]
        print(self.prompts[0])
        self.tokenized_prompts = torch.cat([clip.tokenize(p) for p in self.prompts]).to(self.device)  # (n_cls, n_tkn)
    ...
    def forward_backward(self, batch):
        ...
        if prec == "amp":
            with autocast():
                image_features, text_features = model(image, self.tokenized_prompts, label)
            ...
            loss_ce, logits = model(image, label)
            loss = loss_ce
            optim.zero_grad()
            loss = torch.mean(loss)
            loss.backward()
    ...
    def test(self, split=None):
        ...
        for batch_idx, batch in enumerate(tqdm(data_loader)):
            ...
            output = self.model(input, self.tokenized_prompts)
