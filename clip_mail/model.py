...
class ResidualAttentionBlock_MaPLe(nn.Module):
    ...
    def forward(self, inputs):
        ...
        x = inputs[0]
        counter = inputs[1]
        ln_proj_layers_mlp = inputs[2]
        ln_proj_layers_att = inputs[3]
        mlp_proj_layers_mlp = inputs[4]
        att_proj_layers_att = inputs[5]
        if counter > len(att_proj_layers_att) - 1:
            x = x + self.attention(self.ln_1(x))
            x = x + self.mlp(self.ln_2(x))
            counter = counter + 1
        else:
            att_proj = att_proj_layers_att[counter]
            mlp_proj = mlp_proj_layers_mlp[counter]
            ln_att_proj = ln_proj_layers_att[counter]
            ln_mlp_proj = ln_proj_layers_mlp[counter]
            counter = counter + 1
            # x = x + self.attention(att_proj(self.ln_1(x), self.text_layer, counter))
            # x = x + self.mlp(mlp_proj(self.ln_2(x), self.text_layer, counter))
            #
            x = x + att_proj(self.attention(ln_att_proj(self.ln_1(x), self.text_layer, counter)), self.text_layer, counter)
            x = x + mlp_proj(self.mlp(ln_mlp_proj(self.ln_2(x), self.text_layer, counter)), self.text_layer, counter)
            #
            # x = x + self.attention(self.ln_1(x))
            # x = x + self.mlp(self.ln_2(x))

        return [x, counter, ln_proj_layers_mlp, ln_proj_layers_att, mlp_proj_layers_mlp, att_proj_layers_att]
...
class VisionTransformer_MaPLe(nn.Module):
    ...
    def forward(self, x: torch.Tensor, ln_proj_layers_mlp,
        ln_proj_layers_att, mlp_proj_layers_mlp,  att_proj_layers_att, last_ln=None):
        x = self.conv1(x)  # shape = [*, width, grid, grid]
        x = x.reshape(x.shape[0], x.shape[1], -1)  # shape = [*, width, grid ** 2]
        x = x.permute(0, 2, 1)  # shape = [*, grid ** 2, width]
        x = torch.cat(
        ...
        outputs = self.transformer([x, 0,  ln_proj_layers_mlp,
        ln_proj_layers_att, mlp_proj_layers_mlp,  att_proj_layers_att])  # third argument is counter
        x = outputs[0]
        x = x.permute(1, 0, 2)  # LND -> NLD
